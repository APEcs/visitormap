...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	/**
	 * Checks if it is the first time the user logs in.
	 * @param unknown $identity
	 * @return boolean
	 */
	public function is_first_login($identity)
	{
		
		$query = $this->db->select($this->identity_column . ', pass_changed')
		->where($this->identity_column, $this->db->escape_str($identity))
		->limit(1)
		->get($this->tables['users']);
	...
	/**
	 * Changes the parameter that says if it is first login
	 * returns true, if success, false, if things went wrong
	 * Expects 1 or 0
	 * Returns TRUE, if success, FALSE if failed
	 * @param unknown $value
	 * @return boolean
	 */
	public function set_pass_changed($value, $id)
	{
		if (empty($value) || empty($id) || $value < 0 || $value > 1)
		{
			return FALSE;
		} 
		
		$data = array('pass_changed' => $value);
